...
ggplot(d, aes(x=x, y=y, fill=value)) + geom_tile() + coord_fixed() + theme_bw() + scale_fill_gradientn(colors=myPalette(100),name="mean R2") + annotate("text", x=monotigs$V2+(pos_length/25), y=monotigs$V2+(pos_length/150), label=monotigs$V1, angle=0, size=1.5) + geom_vline(xintercept=36843744+194021, color="green", linetype="dashed") + geom_hline(yintercept=36843744+194021, color="green", linetype="dashed") + geom_vline(xintercept=monotigs$V2, linetype="dotted", size=0.25) + geom_hline(yintercept=monotigs$V2, linetype="dotted", size=0.25) + xlim(start_pos, end_pos) + ylim(start_pos, end_pos)
